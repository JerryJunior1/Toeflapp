...
export default function TatianaAcademicDiscussion() {
  ...
  const fetchAvgScores = async () => {
    try {
      ...
      for (const row of data) {
        ...
        if (!taskId || !taskId.startsWith("tatiana-academic-")) continue;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/grade", {
        ...
        body: JSON.stringify({
          taskId: `tatiana-academic-${selectedTestNum}`,
          taskType: "academic-discussion",
          promptData: academicTask,
  ...
  // ── SELECT ───────────────────────────────────────────────────────────────
  if (phase === "select") {
    const validTests = tatianaTests.filter((t) => t.writing?.academicDiscussion);
    return (
      <div className="w-full max-w-[900px] mx-auto flex flex-col gap-6">
        <div>
          ...
          <h1 className="font-headline text-[32px] font-bold text-on-surface">Academic Discussion</h1>
          <p className="text-[16px] text-on-surface-variant mt-2">
            Read the professor's prompt and the students' posts, then write your own response within the 10-minute time limit. Your response will be graded by AI.
          </p>
        </div>
      ...
      <div className="w-full max-w-[1200px] mx-auto h-[calc(100vh-140px)] flex gap-6">
        ...
        <div className="w-1/2 flex flex-col gap-4 h-full">
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div>
              <p className="text-[12px] font-bold text-on-surface-variant uppercase tracking-wider">Test {selectedTestNum} — Academic Discussion</p>
          ...
          <div className="card rounded-xl flex-1 overflow-y-auto border border-surface-variant bg-surface-container-low">
            <div className="p-6 border-b border-surface-variant flex items-center justify-between sticky top-0 bg-surface-container-low z-10 shadow-sm">
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-primary">description</span>
                <span className="font-bold text-on-surface">Discussion Board</span>
              </div>
              {timeLeft !== null && (
            ...
            <div className="p-6 flex flex-col gap-8">
              ...
              <div className="flex gap-4">
                ...
                <div className="bg-surface-variant/30 rounded-2xl rounded-tl-none p-5 border border-surface-variant flex-1">
                  <p className="text-[13px] font-bold uppercase tracking-wider text-on-surface-variant mb-2">Professor</p>
                  <p className="text-[15px] text-on-surface leading-relaxed">{academicTask?.professorPrompt}</p>
                </div>
              </div>

              {/* Student 1 */}
              <div className="flex gap-4">
                <div className="w-10 h-10 rounded-full bg-[#0d7a5f]/20 flex items-center justify-center shrink-0">
                  <span className="material-symbols-outlined text-[#0d7a5f] text-[20px]">person</span>
                </div>
                <div className="bg-[#0d7a5f]/5 rounded-2xl rounded-tl-none p-5 border border-[#0d7a5f]/20 flex-1">
                  <p className="text-[13px] font-bold uppercase tracking-wider text-[#0d7a5f] mb-2">{academicTask?.student1.name}</p>
                  <p className="text-[15px] text-on-surface leading-relaxed">{academicTask?.student1.post}</p>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div className="bg-secondary/5 rounded-2xl rounded-tl-none p-5 border border-secondary/20 flex-1">
                  <p className="text-[13px] font-bold uppercase tracking-wider text-secondary mb-2">{academicTask?.student2.name}</p>
                  <p className="text-[15px] text-on-surface leading-relaxed">{academicTask?.student2.post}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card rounded-xl flex-1 border border-surface-variant flex flex-col overflow-hidden">
            <div className="p-4 border-b border-surface-variant bg-surface-container-lowest flex justify-between items-center">
              <div className="flex items-center gap-4">
                <span className="text-[14px] font-bold text-on-surface-variant">Your Contribution</span>
                <span className={`text-[12px] font-bold px-2 py-1 rounded ${userInput.trim().split(/\s+/).length < 100 ? 'bg-error/10 text-error' : 'bg-[#0d7a5f]/10 text-[#0d7a5f]'}`}>
            ...
            <textarea
              ...
              placeholder="Express and support your opinion (at least 100 words)..."
        ...
        <div className="flex items-center justify-between">
           <div className="flex items-center gap-3">
             ...
             <div>
               ...
               <h1 className="font-headline text-[24px] font-bold text-on-surface">Test {selectedTestNum} — Academic Discussion</h1>
             </div>
           </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="card rounded-xl p-6 border border-surface-variant">
               ...
               <p className="text-[14px] text-on-surface leading-relaxed whitespace-pre-wrap">{academicTask?.sampleAnswer}</p>
            </div>
