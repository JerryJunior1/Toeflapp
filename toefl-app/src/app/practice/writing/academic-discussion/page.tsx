...
export default function AcademicDiscussion() {
  ...
  const handleSelectTask = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/academic-tasks?id=${id}`);
      const json = await res.json();
      ...
      setSelectedTask({
        ...
        student1Response: json.student_1_response,
        student2Name: json.student_2_name,
        student2Response: json.student_2_response,
      });
    ...
    <div className="w-full max-w-[var(--spacing-container-max)] mx-auto grid grid-cols-1 md:grid-cols-12 gap-[var(--spacing-gutter)]">
        ...
        <div className="md:col-span-5 flex flex-col gap-6">
          <div className="card rounded-xl p-8 flex flex-col gap-4 sticky top-24">
            ...
            <h1 className="font-headline text-[32px] font-bold text-on-surface">{selectedTask.topic}</h1>
        ...
        <div className="md:col-span-7 flex flex-col gap-6">
          <div className="card rounded-xl p-8">
            ...
            <div className="mb-8 pl-6 border-l-2 border-surface-variant">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-8 h-8 rounded-full bg-surface-variant text-on-surface flex items-center justify-center font-bold text-[12px]">{selectedTask.student1Name.charAt(0)}</div>
                <div>
                  <div className="text-[14px] font-bold text-on-surface">{selectedTask.student1Name}</div>
            ...
            <div className="mb-8 pl-6 border-l-2 border-surface-variant">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-8 h-8 rounded-full bg-surface-variant text-on-surface flex items-center justify-center font-bold text-[12px]">{selectedTask.student2Name.charAt(0)}</div>
                <div>
                  <div className="text-[14px] font-bold text-on-surface">{selectedTask.student2Name}</div>
                  <div className="text-[12px] text-on-surface-variant">Today, 11:30 AM</div>
                </div>
              </div>
              <p className="text-[16px] text-on-surface-variant leading-relaxed pl-11">
                {selectedTask.student2Response}
              </p>
            </div>
            ...
            <div className="mt-8">
              ...
              <div className="flex justify-between items-center mt-4">
                ...
                <button 
                  ...
                  className="bg-primary text-white font-medium text-[14px] px-6 py-2 rounded hover:bg-primary-container transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? (
                    <>Grading... <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span></>
                  ) : (
                ...
                {gradingResult.idealResponse && (
                  <div className="bg-[#1a73e8]/5 p-6 rounded-xl border border-[#1a73e8]/20 mt-6">
                    ...
                    <p className="text-[14px] text-on-surface-variant mb-4 italic">
                      This is a 5/5 version of your response, based on your original ideas but with corrected grammar and improved vocabulary.
                    </p>
